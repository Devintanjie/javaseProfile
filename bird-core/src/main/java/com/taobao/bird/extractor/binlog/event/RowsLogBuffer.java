...
/**
 * @desc
 * @author junyu 2015年10月2日下午1:04:46
 * @version
 **/
public final class RowsLogBuffer {
     ...
     * Rows_log_event::print_verbose_one_row
     */
    public final Serializable nextValue(final int type, final int meta, final boolean isBinary) {
     ...
     */
    static int mysqlToJavaType(int type, final int meta, final boolean isBinary) {
        int javaType;
     ...
     */
    final Serializable fetchValue(int type, final int meta, final boolean isBinary) {
        int len = 0;
        ...
        switch (type) {
            ...
            case LogEvent.MYSQL_TYPE_DATETIME2: {
                ...
                int s = (int) (hms % (1 << 6));
                String ts = String.format("%04d-%02d-%02d %02d:%02d:%02d", y, m, d, h, minute, s);
                String microSecond = stringFill(String.valueOf(frac), meta, '0', true);
                if (meta >= 1) microSecond = microSecond.substring(0, meta);
                String str = new StringBuilder(ts).append('.').append(microSecond).toString();
                if (y < 1000 || m == 0 || d == 0) {
                    value = str;
                    javaType = Types.TIMESTAMP;
                } else {
